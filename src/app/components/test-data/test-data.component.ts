...
export class TestDataComponent implements OnInit {
  ...
  generateDynamicBenefitsTable(quote) {
    ...
    quote.forEach((category) => {
      ...
      // Add mandatory and optional benefits
      ...
      categorySections.push({ category, benefitsTable });
    });

    return categorySections; // Return as an array of objects
  }
  ...
  generateAgeBandTables(quoteData) {
    return this.ageBandAndMafData(quoteData.quotes[0].data).map((category) => {
      let ageBandTable;
      const content = [];

      // Check if MAF data is available and add MAF Table
      ...
      return { category_name: category.category_name, ageBandTable: content };
    });
  }
  ...
  async generateDocument(quoteData: any) {
    ...
    const benefitsTables = this.generateDynamicBenefitsTable(quoteData.quotes);
    const ageBandTable = this.generateAgeBandTables(quoteData);

    // const documentSections = [];
    // console.log(benefitsTables);
    // benefitsTables.forEach((benefits) => {
    //   // Find corresponding age band table
    //   console.log(ageBandTable);
    //   const ageBand = ageBandTable.find((table) => table.category_name === benefits.category.category_name);

    //   documentSections.push(
    //     new Paragraph({ text: `Category: ${benefits.category.category_name}`, heading: HeadingLevel.HEADING_1 }),
    //     benefits.benefitsTable, // Add benefits table
    //     ageBand// Remove unnecessary spread operator
    //   );
    // });
    ...
    benefitsTables.forEach((benefits) => {
      ...
      // Iterate over all category names in benefits.data
      benefits.category.data?.forEach((dataItem) => {
        const categoryName = dataItem.category_name; // Extract category_name

        // Find the corresponding age band table
        const ageBand = ageBandTable.find((table) => table.category_name === categoryName);
        console.log(benefits.benefitsTable);
        // Add category header
        documentSections.push(
          new Paragraph({ text: `Category: ${categoryName}`, heading: HeadingLevel.HEADING_1 }),
          benefits.benefitsTable // Add benefits table (only once per benefits object)
        );

        // Add the corresponding ageBandTable contents if found
        if (ageBand && ageBand.ageBandTable) {
          documentSections.push(...ageBand.ageBandTable);
        }
      });
    });
